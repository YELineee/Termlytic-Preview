<template>
  ...
  >
    ...
    <div class="flex items-center justify-between mb-4 flex-shrink-0">
      <div class="flex items-center space-x-2">
        ...
        <h3 class="text-sm font-medium uppercase tracking-wider" 
            :style="{ color: 'var(--textPrimary)' }">WEEKLY ACTIVITY</h3>
      </div>
      <div class="text-xs" :style="{ color: 'var(--textTertiary)' }">Last 7 days</div>
    </div>

    <!-- Activity Heatmap -->
    <div class="rounded-lg p-3 mb-4 flex-shrink-0"
         :style="{ 
      ...
      <div class="grid grid-cols-7 gap-1">
        <div
          ...
          :style="{
            ...
            color: selectedDayIndex === index || day.intensityPercentage > 50 ? 'var(--textPrimary)' : getDayColor(index),
    ...
    <div class="rounded-lg p-3 mb-4 flex-shrink-0 flex items-center"
         :style="{ 
           backgroundColor: 'var(--bgTertiary)', 
           border: '1px solid var(--borderSecondary)',
           minHeight: '80px'
      ...
      <div class="w-full">
        <div class="text-xs mb-2" :style="{ color: 'var(--textTertiary)' }">Selected Day Details</div>
        <div v-if="selectedDay" class="flex items-center justify-between">
          <div class="flex items-center space-x-2">
            ...
            ></div>
            <span class="text-sm font-medium" :style="{ color: 'var(--textPrimary)' }">{{ selectedDay.name }}</span>
          </div>
          <div class="text-right">
            <div class="text-lg font-bold" :style="{ color: 'var(--textPrimary)' }">{{ selectedDay.count }}</div>
            <div class="text-xs" :style="{ color: 'var(--textTertiary)' }">{{ selectedDay.weekPercentage.toFixed(1) }}% of week</div>
          </div>
        </div>
    ...
    <div class="mt-auto pt-3 flex-shrink-0" 
      ...
      <div class="grid grid-cols-2 gap-4 text-xs">
        <div class="flex justify-between">
          ...
          <span class="font-medium" :style="{ color: 'var(--textSecondary)' }">{{ mostActiveDay }}</span>
        </div>
        <div class="flex justify-between">
          <span :style="{ color: 'var(--textTertiary)' }">Total</span>
          <span class="font-medium" :style="{ color: 'var(--textSecondary)' }">{{ weekData.reduce((a, b) => a + b, 0) }}</span>
        </div>
      </div>
    </div>
  </div>
